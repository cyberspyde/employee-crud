...
const buildDocFileName = (employee: Employee) => {
  const nameSegment = makeSafeFileSegment(
...
export default function EmployeeList({
  ...
  const cityOptions = useMemo(() => {
    ...
    employees.forEach((employee) => {
      ...
      if (filters.country !== "all" && employeeCountry !== filters.country)
        return;
  ...
  useEffect(() => {
    if (
      filters.country !== "all" &&
      !countryOptions.some((option) => option.value === filters.country)
    ) {
  ...
  useEffect(() => {
    if (
      filters.city !== "all" &&
      !cityOptions.some((option) => option.value === filters.city)
    ) {
  ...
  useEffect(() => {
    ...
    const filtered = filters.languages.filter((language) =>
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter((employee) => {
      const experienceOption = EXPERIENCE_OPTIONS.find(
        (option) => option.id === filters.experience,
      );
      ...
      if (filters.country !== "all") {
        const employeeCountry =
          employee.address_country?.trim().toLowerCase() ?? "";
      ...
      if (filters.languages.length > 0) {
        ...
        const matches = filters.languages.every((language) =>
          skillSet.has(language),
        );
  ...
  const toggleSelectAll = useCallback(() => {
    ...
    setSelectedEmployees((prev) => {
      const next = new Set(prev);
      const everySelected = filteredEmployees.every((employee) =>
  ...
  const handleExportWord = useCallback(async () => {
    ...
    try {
      ...
      // @ts-ignore - file-saver types issue
      ...
      FileSaver.saveAs(blob, `employees-word-${timestamp}.zip`);
    } catch (error) {
  ...
  if (loading) {
    return (
      <motion.div
        ...
        className="space-y-5"
      >
        <div className="h-24 animate-pulse rounded-3xl border border-slate-200 dark:border-slate-700/70 bg-white/80 dark:bg-slate-800/70 shadow-inner shadow-slate-900/5" />
        {[...Array(4)].map((_, index) => (
          <div
            key={index}
            className="h-32 animate-pulse rounded-3xl border border-slate-200 dark:border-slate-700/60 bg-white/70 dark:bg-slate-800/60 shadow-inner shadow-slate-900/5"
          />
        ))}
      </motion.div>
    );
  ...
  return (
    <div className="space-y-8">
      <motion.section className="relative overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-700/70 bg-white/95 dark:bg-slate-800/90 p-6 shadow-2xl shadow-slate-900/10 sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Tanlangan xodimlarni kuzating, kartalar orqali tezkor ma'lumot
              oling va kerakli amalni bitta klik bilan bajaring.
            </p>
          </div>
          <div className="grid w-full gap-4 rounded-3xl border border-slate-200 dark:border-slate-700/70 bg-white/90 dark:bg-slate-800/80 p-5 text-sm text-slate-600 dark:text-slate-400 shadow-sm shadow-slate-900/5 sm:grid-cols-3 lg:w-auto">
      ...
      <motion.section
        ...
        className="rounded-3xl border border-slate-200 dark:border-slate-700/70 bg-white/95 dark:bg-slate-800/90 shadow-2xl shadow-slate-900/5"
      >
        <header className="flex flex-col gap-4 border-b border-slate-200 dark:border-slate-700/70 px-6 py-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400">
              ...
              {selectedVisibleEmployees.length > 0 &&
                ` � ${selectedVisibleEmployees.length} ta tanlandi`}
            </p>
          </div>
              ...
              <button
                ...
                className={`inline-flex items-center space-x-2 rounded-2xl border px-4 py-2 text-sm font-semibold transition ${
                  hasExportCandidates && exporting !== "word"
                    ...
                    : "cursor-not-allowed border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-700 text-slate-400"
                }`}
              >
                {exporting === "excel" ? (
        ...
        <div className="space-y-4 px-4 pb-6 pt-4 sm:px-6">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 dark:border-slate-700/80 bg-slate-50 dark:bg-slate-900/70 px-4 py-3 text-sm">
          ...
          <div className="space-y-3 rounded-2xl border border-slate-200 dark:border-slate-700/80 bg-slate-50 dark:bg-slate-900/60 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              {hasCustomFilters && (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="inline-flex items-center space-x-2 rounded-2xl border border-orange-200 bg-orange-50 px-4 py-2 text-sm font-semibold text-orange-600 shadow-sm transition hover:bg-orange-100 hover:text-orange-700"
                >
            ...
            <div className="grid gap-4 lg:grid-cols-4">
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateFilter(
                      "experience",
                      event.target.value as ExperienceOption["id"],
                    )
                  }
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-700/70 bg-white dark:bg-slate-800 px-3 py-3 text-sm text-slate-700 dark:text-slate-300 shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10"
                >
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-700/70 bg-white dark:bg-slate-800 px-3 py-3 text-sm text-slate-700 dark:text-slate-300 shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10"
                  disabled={
                    filters.country !== "all" && cityOptions.length === 0
                  }
                >
                        ...
                        <button
                          ...
                          className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold transition ${
                            active
                              ...
                              : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:border-blue-200 hover:text-blue-600"
                          }`}
                        >
                          {option.label}
                        </button>
          ...
          {filteredEmployees.length === 0 ? (
            <div className="rounded-3xl border border-slate-200 dark:border-slate-700/70 bg-white/95 dark:bg-slate-800/90 p-12 text-center shadow-inner shadow-slate-900/5">
              <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-700 text-slate-400">
                <Users className="h-8 w-8" />
              </div>
              <h3 className="mt-4 text-lg font-semibold text-slate-900 dark:text-white">
                Mos keladigan xodimlar topilmadi
              </h3>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                Filtrlarni o'zgartiring yoki tiklash tugmasidan foydalaning.
              </p>
              {hasCustomFilters && (
                <button
                  type="button"
                ...
                {filteredEmployees.map((employee, index) => {
                  ...
                  return (
                    <motion.article
                      ...
                      className={`group relative overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-700/70 bg-white/95 dark:bg-slate-800/90 p-5 shadow-sm shadow-slate-900/5 transition hover:border-blue-200 hover:shadow-xl ${
                        isSelected
                          ? "border-blue-300 shadow-[0_25px_60px_-45px_rgba(59,130,246,0.65)]"
                          : ""
                      }`}
                    >
                    {isSelected && (
                    ...
                    <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="space-y-3">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className="rounded-full bg-slate-100 dark:bg-slate-700 px-2.5 py-0.5 text-xs font-medium text-slate-500 dark:text-slate-400">
                            ...
                            <div className="grid gap-3 text-sm text-slate-600 dark:text-slate-400 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            <div className="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-700 px-3 py-1 text-xs font-semibold text-slate-500 dark:text-slate-400">
                            ...
                            {employee.skills?.length ? (
                              <div className="flex flex-wrap gap-2">
                                {employee.skills.map((skill) => (
                                  <span
                                    key={skill}
                                    className="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-700 px-3 py-1 text-xs font-semibold text-slate-500 dark:text-slate-400"
                                  >
                                    {skill}
                                  </span>
                                ))}
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="rounded-xl border border-slate-200 dark:border-slate-700/70 bg-white/90 dark:bg-slate-800/80 px-3 py-2 text-slate-500 dark:text-slate-400 transition hover:border-blue-200 hover:text-blue-600"
                        >
                          <Eye className="h-4 w-4" />
                        </button>
                        <button
                          onClick={() => onEdit(employee)}
                          className="rounded-xl border border-slate-200 dark:border-slate-700/70 bg-white/90 dark:bg-slate-800/80 px-3 py-2 text-slate-500 dark:text-slate-400 transition hover:border-emerald-200 hover:text-emerald-600"
                        >
                          <Pencil className="h-4 w-4" />
                        </button>
                        <button
                          onClick={() => onDelete(employee.id)}
                          className="rounded-xl border border-slate-200 dark:border-slate-700/70 bg-white/90 dark:bg-slate-800/80 px-3 py-2 text-slate-500 dark:text-slate-400 transition hover:border-rose-200 hover:text-rose-600"
                        >
